...
class VolumesBackupsV2Test(base.BaseVolumeAdminTest):
    ...
    def test_volume_backup_create_get_detailed_list_restore_delete(self):
        ...
        # Restore backup, must specify a volume id.
        restore = self.backups_adm_client.restore_backup(
            backup['id'], volume_id=self.volume['id'])['restore']

        # Delete backup
        self.addCleanup(self.admin_volume_client.delete_volume,
                        restore['volume_id'])
        self.assertEqual(backup['id'], restore['backup_id'])
        self.backups_adm_client.wait_for_backup_status(backup['id'], 'available')
        waiters.wait_for_volume_status(self.admin_volume_client, restore['volume_id'], 'available')
